...
window.ctRenderLine = function (idx) {
  ...
  window.initClassTrialDialogue = function () {
    ...
    if (window.__ctStartTrio) {
      ...
      // Встаём не на саму сцену, а на последнюю реплику перед ней: между
      // ними лежат служебные записи, которые заводят нужную тему. Прыжок
      // сразу в сцену их проглатывал, и она шла в тишине.
      while (start > 0 && !(lines[start - 1] && lines[start - 1].text)) start--;
      if (start > 0) start--;
    } else {
